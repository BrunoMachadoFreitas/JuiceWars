...
public class MapController : MonoBehaviour
{
    ...
    void ChunkChecker()
    {
        ...
        else if (pm.movement.y > 0 && pm.movement.x == 0)
        {
    ...
    void ChunkOptimzer()
    {
        ...
        foreach (GameObject chunk in spawnedChunks)
        {
            if (Player_Main.instance) { 
                opDist = Vector3.Distance(Player_Main.instance.transform.position, chunk.transform.position);
                if (opDist > maxOpDist)
                {
